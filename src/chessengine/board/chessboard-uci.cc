...
namespace board {
    ...
    Move Chessboard::parse_uci_move(std::string uci_move) {
        ...
        Position start_position((File) uci_move.at(0) - 'a',
                (Rank) uci_move.at(1) - '1');
        Position end_position((File) uci_move.at(2) - 'a',
                (Rank) uci_move.at(3) - '1');
        ...
        if (move.piece_ == PieceType::PAWN) {
            ...
            if (start_position.rank_get() + direction * 2
                == end_position.rank_get()) {
            ...
            if (uci_move.size() >= 5) {
                //promotion
                move.promotion_ = std::make_optional<PieceType>(
                        char_to_piece(toupper(uci_move.at(4))));
            }
            return move;
        }
    ...
    std::shared_ptr<Chessboard> Chessboard::parse_uci(
            std::string uci_position) {
        ...
        if (s == "position") {
            ...
            throw std::runtime_error("position string invalid");
        }
        ...
        if (s == "fen") {
            try {
                ...
                std::cerr << "error while recreating board position from uci";
            }
        } else if (s == "startpos") {
        ...
        try {
            ...
            std::cerr << "error while recreating board"
                         << "position from uci move list :" << e.what() << "\n";
        }
        return nullptr;
    }
}