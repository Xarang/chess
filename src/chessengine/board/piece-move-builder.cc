...
namespace board {
 //some quick constructors for most moves
    ...
    void MoveBuilder::set_castling_decomposition(std::vector<Move>& moves, const Piece& p, bool king_castling) {
        ...
        for (File f = p.position_.file_get(); f != end_file; f = f + direction) {
            ...
            moves.back().end_position_ = Position(f + direction, p.position_.rank_get());
