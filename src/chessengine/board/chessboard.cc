...
namespace board {
    ...
    bool Chessboard::occupied_by(board::Color myColor,
            board::PieceType myType, board::Position myPos) {
    ...
    bool Chessboard::is_move_legal(Move &move, bool check_self_check) {
        ...
        if (past_moves_en_passant_target_squares_.back().has_value() &&
            ...
            //this is the place where en passant moves are flagged are so.
            // no en passant moves should be created before this.
        ...
        if (piece) {
            ...
            //do not consider moves that end on a square occupied by an
            // allied piece
            if (piece->color_ == whose_turn_is_it())
                return false;
        } else if (move.is_capture_ && !move.is_en_passant_) {
            //do not consider capture moves that end on an empty square...
            // except for en-passant
            return false;
        }
        //check if the movement is 'correct' relative to the piece type,
        // and potential piece blocking the movement
        if (!MoveLegalityChecker::is_move_legal(*this, move)) {
            return false;
        }
        //now that we know the move is 'valid', we check if it is illegal
        // due to " can not check yourself " clause
        ...
        if (check_self_check) {
            ...
            if (move.is_king_castling_ || move.is_queen_castling_) {
                ...
                auto king = pieces_[{PieceType::KING, whose_turn_is_it()}]
                        .front();
                std::vector<Move> moves_to_castling;
                MoveBuilder::set_castling_decomposition(moves_to_castling,
                        *king, move.is_king_castling_);
                bool was_checked_on_the_way = false;
                for (auto it = moves_to_castling.begin();
                    it != moves_to_castling.end(); it++) {
                ...
                for (auto it = moves_to_castling.rbegin();
                    it != moves_to_castling.rend(); it++) {
                    //we know that these tiles are free
                    undo_move(*it, false);
                }
    ...
    void Chessboard::generate_legal_moves(std::list<Move>& allMoves,
            bool check_self_check) {
        //std::list<Move> allMoves;
        //build the list of all "potential" moves, not accounting for
        // OOB and blocked paths
    ...
    bool Chessboard::is_check() {
        ...
        //generate legal moves for opponent and check if one on them captures
        // your king.
        change_turn();
        std::list<Move> opponent_moves;
        //this false means that this call to generate_legal_moves will not
        // check for check itself (since the other player has initiative anyway)
        generate_legal_moves(opponent_moves, false);

        change_turn();

        auto king = pieces_[{PieceType::KING,
            whose_turn_is_it()}].front();
    ...
    bool Chessboard::is_checkmate() {
        //is check + project all your legal moves as long as you don't find one
        // in which you are not checked. if you can't find one,
        // you are checkmated
        if (!is_check()) {
            return false;
        }
    ...
    std::string Chessboard::to_string() const {
        //outputs a FEN string representing the board
