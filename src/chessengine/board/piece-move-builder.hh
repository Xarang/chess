...
namespace board {
    class MoveBuilder {
        public:
            static inline void set_basic_move(Move& move, const Piece& p, Position target) {
                move.start_position_ = p.position_;
                move.end_position_ = target;
                move.piece_ = p.type_;
            }

            static inline void set_basic_capture(Move& move, const Piece& p, Position target) {
                move.start_position_ = p.position_;
                move.end_position_ = target;
                move.piece_ = p.type_;
            ...
            static inline void set_with_promotion(Move& move, const Move& original, PieceType type) {
                //only place where move copying should happen (only copying relevant fields)
            ...
            static inline void set_double_pawn_push(Move& move, const Piece& p) {
                move.start_position_ = p.position_;
            ...
            static inline void set_king_castling(Move& m, const Piece& p) {
                MoveBuilder::set_basic_move(m, p, Position(File::G, p.position_.rank_get()));
                m.is_king_castling_ = true;
            }
            static inline void set_queen_castling(Move& m, const Piece& p) {
                MoveBuilder::set_basic_move(m, p, Position(File::C, p.position_.rank_get()));
                m.is_queen_castling_ = true;
            }

            static void set_castling_decomposition(std::vector<Move>&, const Piece& p, bool king_castling);
    };
}